...
void CalibrationToolkitBase::loadCalibResultSlot() {
  QString filename = QFileDialog::getOpenFileName(this, "Load Calib", QString(),
                                                  "YAML (*.yaml)");
  if (!filename.isEmpty()) {
    cv::FileStorage fs(filename.toStdString(), cv::FileStorage::READ);
    if (!fs.isOpened()) {
      std::cout << "Failed to open settings file at: " << filename.toStdString()
                << std::endl;
    } else
      std::cout << "success to open file at: " << filename.toStdString()
                << std::endl;
...
void CalibrationToolkitBase::saveCalibResultSlot() {
  QString filename = QFileDialog::getSaveFileName(this, "Save Calib", QString(),
                                                  "YAML (*.yaml)");
  if (!filename.isEmpty()) {
...
void CalibrationToolkitBase::setResultShow(cv::Mat result, QTableWidget *show) {
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < m; j++) {
      show->setItem(
          i, j,
          new QTableWidgetItem(QString("%1").arg(result.at<double>(i, j))));
    }
  }
}

void CalibrationToolkitBase::readResultShow(cv::Mat &result,
                                            QTableWidget *show) {
  int i, n;
  int j, m;
  n = show->rowCount();
...
QVector<double> CalibrationToolkitBase::convertMatrix2Euler(cv::Mat mat) {
  ...
  euler[1] = atan2(-mat.at<double>(2, 0),
                   sqrt(mat.at<double>(2, 1) * mat.at<double>(2, 1) +
                        mat.at<double>(2, 2) * mat.at<double>(2, 2)));
...
bool CalibrateCameraBase::refreshImage() {
  ...
  if (calibimage.type() == CV_8UC3) {
    QImage img(calibimage.data, calibimage.cols, calibimage.rows,
               calibimage.step, QImage::Format_RGB888);
    img = img.rgbSwapped();
    calibimageshow->setPixmap(QPixmap::fromImage(img));
    calibimageshow->resize(img.size());
  } else if (calibimage.type() == CV_8UC1) {
    QImage img(calibimage.data, calibimage.cols, calibimage.rows,
               calibimage.step, QImage::Format_Indexed8);
  ...
  imagesizeshow->setText(
      QString("%1 * %2").arg(imagesize.height).arg(imagesize.width));
  return 1;
}

bool CalibrateCameraBase::loadCalibResult(cv::FileStorage &fs) {
  cameraextrinsicmat = loadSavedMat(fs, CAMERAEXTRINSICMAT);

  setResultShow(cameraextrinsicmat, cameraextrinsicshow);
  cameramat = loadSavedMat(fs, CAMERAMAT);
  setResultShow(cameramat, cameramatshow);
  distcoeff = loadSavedMat(fs, DISTCOEFF);
  setResultShow(distcoeff, distcoeffshow);
  fs[IMAGESIZE] >> imagesize;

  printf("CameraExtrinsicMat:[%d %d]\n", cameraextrinsicmat.cols,
         cameraextrinsicmat.rows);
  printf("CameraMat:[%d %d]\n", cameramat.cols, cameramat.rows);
  printf("DistCoeff:[%d %d]\n", distcoeff.cols, distcoeff.rows);
...
cv::Mat CalibrateCameraBase::getCameraMat() { return cameramat; }

cv::Mat CalibrateCameraBase::getDistCoeff() { return distcoeff; }

cv::Size2i CalibrateCameraBase::getImageSize() { return imagesize; }

cv::Mat CalibrateCameraBase::loadSavedMat(cv::FileStorage &fs,
                                          std::string matName) {
  int rows, cols;
  fs[matName]["rows"] >> rows;
  fs[matName]["cols"] >> cols;
  cv::Mat resultMat(rows, cols, CV_64F);
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      resultMat.at<double>(i, j) = fs[matName]["data"][j + i * cols];
    }
  }

  printf("CalibrateCameraBase::loadSavedMat: %s\n", matName.c_str());
  // for (int i = 0; i < rows; ++i)
  // {
  //     for (int j = 0; j < cols; ++j)
  //     {
  //         printf("%0.4f   ", resultMat.at<double>(i, j));
  //     }
  //     printf("\n");
  // }
...
cv::Mat CalibrateCameraChessboardBase::loadSavedMat(cv::FileStorage &fs,
                                                    std::string matName) {
  int rows, cols;
  fs[matName]["rows"] >> rows;
  fs[matName]["cols"] >> cols;
  ...
  printf("CalibrateCameraChessboardBase::loadSavedMat: %s\n", matName.c_str());
  // for (int i = 0; i < rows; ++i)
  // {
  //     for (int j = 0; j < cols; ++j)
  //     {
...
cv::Mat CalibrateCameraVelodyneChessboardBase::loadSavedMat(
    cv::FileStorage &fs, std::string matName) {
  int rows, cols;
  fs[matName]["rows"] >> rows;
  fs[matName]["cols"] >> cols;
  cv::Mat resultMat(rows, cols, CV_64F);
  ...
  printf("CalibrateCameraVelodyneChessboardBase::loadSavedMat: %s\n",
         matName.c_str());
  // for (int i = 0; i < rows; ++i)
  // {
  //     for (int j = 0; j < cols; ++j)
...
cv::Mat CalibrateCameraChessboardBase::loadSavedImg(cv::FileStorage &fs,
                                                    std::string matName) {
  int rows, cols;
  fs[matName]["rows"] >> rows;
  fs[matName]["cols"] >> cols;
  cv::Mat resultMat(rows, cols, CV_8UC3);
  printf("CalibrateCameraChessboardBase::loadSavedMat: %s\n", matName.c_str());
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      int tmp0 = fs[matName]["data"]
                   [0 + j * resultMat.step[1] + i * resultMat.step[0]];
      int tmp1 = fs[matName]["data"]
                   [1 + j * resultMat.step[1] + i * resultMat.step[0]];
      int tmp2 = fs[matName]["data"]
                   [2 + j * resultMat.step[1] + i * resultMat.step[0]];

      resultMat.at<cv::Vec3b>(i, j)[0] = tmp0;
      resultMat.at<cv::Vec3b>(i, j)[1] = tmp1;
      resultMat.at<cv::Vec3b>(i, j)[2] = tmp2;
    }
  }
...
CalibrateCameraChessboardBase::CalibrateCameraChessboardBase(
    cv::Size2f patternSize, cv::Size2i patternNum, QWidget *parent)
    : CalibrateCameraBase(parent) {
  patternsize = patternSize;
  printf("############ patternsize:%f %f  patternNum:%d %d\n",
         patternsize.width, patternsize.height, patternNum.width,
         patternNum.height);
...
bool CalibrateCameraChessboardBase::removeCalibData() {
  ...
  QTableWidget *tmpchessboardposeshow =
      (QTableWidget *)(chessboardposeshow->widget(id));
...
bool CalibrateCameraChessboardBase::calibrateSensor() {
  ...
  reprojectionerror = cv::calibrateCamera(grid3dpoints, grid2dpoints, imgsize,
                                          cameramat, distcoeff, rvecs, tvecs);

  if (USINGFIXEDINTRINSTICMATRIX) {
    cameramat.at<double>(0, 0) = 1357.43;
    cameramat.at<double>(0, 2) = 581.86;
    cameramat.at<double>(1, 1) = 1373.99;
    cameramat.at<double>(1, 2) = 350.77;

    distcoeff.at<double>(0, 0) = -0.5577;
    distcoeff.at<double>(0, 1) = 0.8784;
    distcoeff.at<double>(0, 2) = -0.0007;
    distcoeff.at<double>(0, 3) = 0.0010;
    distcoeff.at<double>(0, 4) = -1.0676;
  }
  ...
  for (i = 0; i < n; i++) {
    ...
    setResultShow(chessboardpose,
                  (QTableWidget *)(chessboardposeshow->widget(i)));
  }
  return 1;
}
...
bool CalibrateCameraChessboardBase::loadCalibResult(cv::FileStorage &fs) {
  ...
  QMessageBox::StandardButton button = QMessageBox::question(
      this, "Load Option", "Load Camera Calibration Data?");
  if (button == QMessageBox::Yes) {
    ...
    for (i = 0; i < n; i++) {
      fs[QString("%1_%2").arg(GRID3DPOINTS).arg(i).toStdString()] >>
          grid3dpoints[i];
      fs[QString("%1_%2").arg(GRID2DPOINTS).arg(i).toStdString()] >>
          grid2dpoints[i];

      chessboardposes[i] = loadSavedMat(
          fs, QString("%1_%2").arg(CHESSBOARDPOSE).arg(i).toStdString());
      ...
      chessboardposeshow->addTab(tmpchessboardposeshow,
                                 QString("Chessboard_%1").arg(i));
      setResultShow(chessboardposes[i], tmpchessboardposeshow);

      calibimages[i] = loadSavedImg(
          fs, QString("%1_%2").arg(CHESSBOARDIMAGE).arg(i).toStdString());

      // cv::imshow("calibimages[i]", calibimages[i]);
      // cv::waitKey();
      ...
      if (tmpimage.type() == CV_8UC3) {
        QImage img(tmpimage.data, tmpimage.cols, tmpimage.rows, tmpimage.step,
...
bool CalibrateCameraChessboardBase::saveCalibResult(cv::FileStorage &fs) {
  ...
  QMessageBox::StandardButton button = QMessageBox::question(
      this, "Save Option", "Save Camera Calibration Data?");
  if (button == QMessageBox::Yes) {
...
CalibrateCameraChessboardROS::CalibrateCameraChessboardROS(
    QString topic, u_int32_t queueSize, int interval, cv::Size2f patternSize,
    cv::Size2i patternNum, QWidget *parent)
    : CalibrateCameraChessboardBase(patternSize, patternNum, parent) {
  camerasub =
      new ROSSub<sensor_msgs::ImageConstPtr>(topic, queueSize, interval);
  connect(camerasub, SIGNAL(receiveMessageSignal()), this,
          SLOT(refreshImageSlot()));
...
bool CalibrateCameraChessboardROS::refreshImage() {
  ...
  cameratimestamp = QTime::fromMSecsSinceStartOfDay(
      (msg->header.stamp.sec % (24 * 60 * 60)) * 1000 +
...
bool CalibrateCameraChessboardROS::grabCalibData() {
  ...
  cv::cornerSubPix(
      calibimage, grid2dpoint, cv::Size(11, 11), cv::Size(-1, -1),
      cv::TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 30, 0.1));
  calibimages.push_back(calibimage.clone());
  printf("grid3dpoint.size():%d\n", grid3dpoint.size());
  ...
  chessboardposeshow->addTab(
      tmpchessboardtable, QString("Chessboard_%1").arg(calibimages.size() - 1));
  ...
  if (tmpimage.type() == CV_8UC3) {
    ...
    QScrollArea *scrollarea = new QScrollArea;
    scrollarea->setWidget(tmpcalibimageshow);
    calibimagesshow->addTab(scrollarea,
                            QString("Image_%1").arg(calibimages.size() - 1));
...
CalibrateCameraVelodyneChessboardBase::CalibrateCameraVelodyneChessboardBase(
  ...
  velodynesplitter->setStretchFactor(
      velodynesplitter->indexOf(calibvelodynesshow), 1);
}
...
void CalibrateCameraVelodyneChessboardBase::extractionResultSlot(
    pcl::PointCloud<pcl::PointXYZI>::Ptr extraction, cv::Mat normal, int id) {
  calibvelodynespoints[id] = extraction;
  calibvelodynesnormals[id] = normal;
  QTableWidget *pointstable =
      (QTableWidget *)(calibvelodynepointstab->widget(id));
  QTableWidget *normalstable =
      (QTableWidget *)(calibvelodynenormalstab->widget(id));
  pointstable->clear();
  normalstable->clear();
  if (extraction != NULL) {
    ...
    for (i = 0; i < n; i++) {
      pointstable->setItem(
          i, 0,
          new QTableWidgetItem(QString("%1").arg(extraction->points[i].x)));
      pointstable->setItem(
          i, 1,
          new QTableWidgetItem(QString("%1").arg(extraction->points[i].y)));
      pointstable->setItem(
          i, 2,
          new QTableWidgetItem(QString("%1").arg(extraction->points[i].z)));
      pointstable->setItem(i, 3,
                           new QTableWidgetItem(QString("%1").arg(
                               extraction->points[i].intensity)));
    }
...
void CalibrateCameraVelodyneChessboardBase::projectPointsSlot() {
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < m; j++) {
      camerapoints.at<double>(j, 0) =
          double(calibvelodynespoints[i]->points[j].x);
      camerapoints.at<double>(j, 1) =
          double(calibvelodynespoints[i]->points[j].y);
      camerapoints.at<double>(j, 2) =
          double(calibvelodynespoints[i]->points[j].z);
    }
    camerapoints =
        camerapoints * invR.t() + cv::Mat::ones(m, 1, CV_64F) * invT.t();
    std::vector<cv::Point2d> planepoints;
    planepoints.resize(m);
    for (j = 0; j < m; j++) {
      double tmpx =
          camerapoints.at<double>(j, 0) / camerapoints.at<double>(j, 2);
      double tmpy =
          camerapoints.at<double>(j, 1) / camerapoints.at<double>(j, 2);
      double r2 = tmpx * tmpx + tmpy * tmpy;
      double tmpdist = 1 + distcoeff.at<double>(0) * r2 +
                       distcoeff.at<double>(1) * r2 * r2 +
                       distcoeff.at<double>(4) * r2 * r2 * r2;
      planepoints[j].x = tmpx * tmpdist +
                         2 * distcoeff.at<double>(2) * tmpx * tmpy +
                         distcoeff.at<double>(3) * (r2 + 2 * tmpx * tmpx);
      planepoints[j].y = tmpy * tmpdist +
                         distcoeff.at<double>(2) * (r2 + 2 * tmpy * tmpy) +
                         2 * distcoeff.at<double>(3) * tmpx * tmpy;
      planepoints[j].x = cameramat.at<double>(0, 0) * planepoints[j].x +
                         cameramat.at<double>(0, 2);
      planepoints[j].y = cameramat.at<double>(1, 1) * planepoints[j].y +
...
bool CalibrateCameraVelodyneChessboardBase::refreshVelodyne() {
  ...
  int count = 100;
  int cut = n / count;
  // printf("\n num:%d point_step:%d cut:%d [%d %d] fields[0].offset:%d\n",
  //     n, calibvelodyne->point_step, cut, calibvelodyne->width,
  //     calibvelodyne->height, calibvelodyne->row_step,
  //     calibvelodyne->fields[0].offset);
  // for(i=0;i < count;i++)
  // {
  //     float * base=(float *)(data+(i * cut)*calibvelodyne->point_step);
  //     printf("[%0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f]", base[0], base[1],
  ...
  glVertexPointer(3, GL_FLOAT, calibvelodyne->point_step,
                  calibvelodyne->data.data() + 4);  // 4
...
bool CalibrateCameraVelodyneChessboardBase::removeCalibData() {
  ...
  planeextractorspoints.erase(planeextractorspoints.begin() + id);

  QTableWidget *tmpvelodynetable =
      (QTableWidget *)(calibvelodynepointstab->widget(id));
  calibvelodynepointstab->removeTab(id);
  delete tmpvelodynetable;

  QTableWidget *tmpvelodynenormaltable =
      (QTableWidget *)(calibvelodynenormalstab->widget(id));
  calibvelodynenormalstab->removeTab(id);
  delete tmpvelodynenormaltable;

  PlaneExtractor *planeextraction =
      (PlaneExtractor *)(calibvelodynesshow->widget(id));
...
double calibrationCameraVelodyneChessboardObjectiveFunc(
    const std::vector<double> &x, std::vector<double> &grad, void *data) {
  CalibrateCameraVelodyneChessboardBase::CameraVelodyneCalibrationData
      *calibdata = reinterpret_cast<CalibrateCameraVelodyneChessboardBase::
                                        CameraVelodyneCalibrationData *>(data);
  ...
  rmat.at<double>(0, 0) = rotation(0, 0);
  rmat.at<double>(0, 1) = rotation(0, 1);
  rmat.at<double>(0, 2) = rotation(0, 2);
  rmat.at<double>(1, 0) = rotation(1, 0);
  rmat.at<double>(1, 1) = rotation(1, 1);
  rmat.at<double>(1, 2) = rotation(1, 2);
  rmat.at<double>(2, 0) = rotation(2, 0);
  rmat.at<double>(2, 1) = rotation(2, 1);
  rmat.at<double>(2, 2) = rotation(2, 2);

  cv::Mat vmat(1, 3, CV_64F);
  vmat.at<double>(0) = x[3];
  vmat.at<double>(1) = x[4];
  vmat.at<double>(2) = x[5];

  int chessboardnum = calibdata->velodynepoints.size();

  cv::Mat normals = calibdata->chessboardnormals * rmat.t();
  cv::Mat points = calibdata->chessboardpoints * rmat.t() +
                   cv::Mat::ones(chessboardnum, 1, CV_64F) * vmat;
  ...
  for (i = 0; i < n; i++) {
    ...
    cv::Mat dists =
        (calibdata->velodynepoints[i] - duppoints) * (normals.row(i).t());
...
double calibrationCameraVelodyneChessboardRotationalObjectiveFunc(
    const std::vector<double> &x, std::vector<double> &grad, void *data) {
  CalibrateCameraVelodyneChessboardBase::CameraVelodyneCalibrationData
      *calibdata = reinterpret_cast<CalibrateCameraVelodyneChessboardBase::
                                        CameraVelodyneCalibrationData *>(data);
...
double calibrationCameraVelodyneChessboardTranslationalObjectiveFunc(
    const std::vector<double> &x, std::vector<double> &grad, void *data) {
  CalibrateCameraVelodyneChessboardBase::CameraVelodyneCalibrationData
      *calibdata = reinterpret_cast<CalibrateCameraVelodyneChessboardBase::
                                        CameraVelodyneCalibrationData *>(data);
  ...
  cv::Mat vmat(1, 3, CV_64F);
  vmat.at<double>(0) = x[0];
  vmat.at<double>(1) = x[1];
  vmat.at<double>(2) = x[2];
...
bool CalibrateCameraVelodyneChessboardBase::calibrateSensor() {
  ...
  for (i = 0; i < n; i++) {
    calibrationdata.chessboardnormals.row(i) =
        normal * chessboardposes[i](cv::Rect(0, 0, 3, 3)).t();
    calibrationdata.chessboardpoints.at<double>(i, 0) =
        chessboardposes[i].at<double>(0, 3);
    calibrationdata.chessboardpoints.at<double>(i, 1) =
        chessboardposes[i].at<double>(1, 3);
    calibrationdata.chessboardpoints.at<double>(i, 2) =
        chessboardposes[i].at<double>(2, 3);
    if (calibvelodynespoints[i] == NULL) {
    ...
    for (j = 0; j < m; j++) {
      calibrationdata.velodynepoints[i].at<double>(j, 0) =
          double(calibvelodynespoints[i]->points[j].x);
      calibrationdata.velodynepoints[i].at<double>(j, 1) =
          double(calibvelodynespoints[i]->points[j].y);
      calibrationdata.velodynepoints[i].at<double>(j, 2) =
          double(calibvelodynespoints[i]->points[j].z);
    }
    calibrationdata.velodynenormals.at<double>(i, 0) =
        calibvelodynesnormals[i].at<double>(0);
    calibrationdata.velodynenormals.at<double>(i, 1) =
        calibvelodynesnormals[i].at<double>(1);
    calibrationdata.velodynenormals.at<double>(i, 2) =
        calibvelodynesnormals[i].at<double>(2);
  }
  if (!flag) {
    return 0;
  }
  ...
  cv::Mat NN =
      calibrationdata.chessboardnormals.t() * calibrationdata.chessboardnormals;
  if (cv::determinant(NN) == 0) {
    QMessageBox::information(this, "Error", "Not enough patterns");
    return 0;
  }
  cv::Mat NM =
      calibrationdata.chessboardnormals.t() * calibrationdata.velodynenormals;
  ...
  calibrationrotationalerror = cv::norm(calibrationdata.velodynenormals -
                                        calibrationdata.chessboardnormals *
                                            calibrationdata.rotationresult.t());
  ...
  x[2] = cameraextrinsicmat.at<double>(2, 3);
  {
    ...
    localopt.set_min_objective(
        calibrationCameraVelodyneChessboardTranslationalObjectiveFunc,
  ...
  cameraextrinsicmat.at<double>(0, 3) = x[0];
  cameraextrinsicmat.at<double>(1, 3) = x[1];
  cameraextrinsicmat.at<double>(2, 3) = x[2];
  qDebug() << x[0] << x[1] << x[2];
...

  {
    ...
    localopt.set_upper_bounds(ub.toStdVector());
    localopt.set_xtol_rel(1e-4);
    localopt.set_min_objective(calibrationCameraVelodyneChessboardObjectiveFunc,
                               (void *)&calibrationdata);
  ...
  cameraextrinsicmat.at<double>(0, 0) = rotation(0, 0);
  cameraextrinsicmat.at<double>(0, 1) = rotation(0, 1);
  cameraextrinsicmat.at<double>(0, 2) = rotation(0, 2);
  cameraextrinsicmat.at<double>(1, 0) = rotation(1, 0);
  cameraextrinsicmat.at<double>(1, 1) = rotation(1, 1);
  cameraextrinsicmat.at<double>(1, 2) = rotation(1, 2);
  cameraextrinsicmat.at<double>(2, 0) = rotation(2, 0);
  cameraextrinsicmat.at<double>(2, 1) = rotation(2, 1);
  cameraextrinsicmat.at<double>(2, 2) = rotation(2, 2);

  cameraextrinsicmat.at<double>(0, 3) = xx[3];
  cameraextrinsicmat.at<double>(1, 3) = xx[4];
  cameraextrinsicmat.at<double>(2, 3) = xx[5];

  calibrationerrorshow->setText(QString("%1, %2")
                                    .arg(calibrationrotationalerror)
                                    .arg(calibrationtranslationalerror));
  setResultShow(cameraextrinsicmat, cameraextrinsicshow);
  return 1;
}

bool CalibrateCameraVelodyneChessboardBase::loadCalibResult(
    cv::FileStorage &fs) {
  CalibrateCameraChessboardBase::loadCalibResult(fs);

  QMessageBox::StandardButton button = QMessageBox::question(
      this, "Load Option", "Load Velodyne Calibration Data?");
  if (button == QMessageBox::Yes) {
    int i, n = (int)fs[CHESSBOARDVIEWNUM];
    calibvelodynespoints.resize(n);
    planeextractorspoints.resize(n);
    ...
    for (i = 0; i < n; i++) {
      cv::Mat tmpmat, tmpmat1;
      tmpmat = loadSavedMat(
          fs, QString("%1_%2").arg(PLANEEXTRACTORSPOINTS).arg(i).toStdString());
      // tmpmat1 = loadSavedMat(fs,
      // QString("%1_%2").arg(VELODYNEPOINTS).arg(i).toStdString());

      int j, m = tmpmat.rows;
      planeextractorspoints[i] = pcl::PointCloud<pcl::PointXYZI>::Ptr(
          new pcl::PointCloud<pcl::PointXYZI>);
      planeextractorspoints[i]->points.resize(m);

      // calibvelodynespoints[i]=pcl::PointCloud<pcl::PointXYZI>::Ptr(new
      // pcl::PointCloud<pcl::PointXYZI>);
      // calibvelodynespoints[i]->points.resize(m);

      QTableWidget *tmpvelodynetable = new QTableWidget;
      calibvelodynepointstab->addTab(tmpvelodynetable,
                                     QString("Velodyne_%1").arg(i));
      tmpvelodynetable->setRowCount(m);
      tmpvelodynetable->setColumnCount(4);
      for (j = 0; j < m; j++) {
        planeextractorspoints[i]->points[j].x = tmpmat.at<double>(j, 0);
        planeextractorspoints[i]->points[j].y = tmpmat.at<double>(j, 1);
        planeextractorspoints[i]->points[j].z = tmpmat.at<double>(j, 2);
        planeextractorspoints[i]->points[j].intensity = tmpmat.at<double>(j, 3);

        // calibvelodynespoints[i]->points[j].x=tmpmat1.at<float>(j,0);
        // calibvelodynespoints[i]->points[j].y=tmpmat1.at<float>(j,1);
        // calibvelodynespoints[i]->points[j].z=tmpmat1.at<float>(j,2);
        // calibvelodynespoints[i]->points[j].intensity=tmpmat1.at<float>(j,3);

        // tmpvelodynetable->setItem(j,0,new
        // QTableWidgetItem(QString("%1").arg(calibvelodynespoints[i]->points[j].x)));
        // tmpvelodynetable->setItem(j,1,new
        // QTableWidgetItem(QString("%1").arg(calibvelodynespoints[i]->points[j].y)));
        // tmpvelodynetable->setItem(j,2,new
        // QTableWidgetItem(QString("%1").arg(calibvelodynespoints[i]->points[j].z)));
        // tmpvelodynetable->setItem(j,3,new
        // QTableWidgetItem(QString("%1").arg(calibvelodynespoints[i]->points[j].intensity)));
      }

      cv::Mat tmpviewerpose;
      tmpviewerpose = loadSavedMat(
          fs, QString("%1_%2").arg(VELODYNEVIEWERPOSE).arg(i).toStdString());
      Eigen::Matrix4d viewerpose;
      viewerpose(0, 0) = tmpviewerpose.at<double>(0, 0);
      viewerpose(0, 1) = tmpviewerpose.at<double>(0, 1);
      viewerpose(0, 2) = tmpviewerpose.at<double>(0, 2);
      viewerpose(0, 3) = tmpviewerpose.at<double>(0, 3);
      viewerpose(1, 0) = tmpviewerpose.at<double>(1, 0);
      viewerpose(1, 1) = tmpviewerpose.at<double>(1, 1);
      viewerpose(1, 2) = tmpviewerpose.at<double>(1, 2);
      viewerpose(1, 3) = tmpviewerpose.at<double>(1, 3);
      viewerpose(2, 0) = tmpviewerpose.at<double>(2, 0);
      viewerpose(2, 1) = tmpviewerpose.at<double>(2, 1);
      viewerpose(2, 2) = tmpviewerpose.at<double>(2, 2);
      viewerpose(2, 3) = tmpviewerpose.at<double>(2, 3);
      viewerpose(3, 0) = tmpviewerpose.at<double>(3, 0);
      viewerpose(3, 1) = tmpviewerpose.at<double>(3, 1);
      viewerpose(3, 2) = tmpviewerpose.at<double>(3, 2);
      viewerpose(3, 3) = tmpviewerpose.at<double>(3, 3);
      radius = PLANEEXTRACTORRADIUS;
      PlaneExtractor *planeextraction =
          new PlaneExtractor(planeextractorspoints[i], i, radius);
      calibvelodynesshow->addTab(planeextraction,
                                 QString("Velodyne_%1").arg(i));
      planeextraction->setCameraPose(viewerpose);
      planeextraction->update();
      connect(planeextraction,
              SIGNAL(extractionResultSignal(
                  pcl::PointCloud<pcl::PointXYZI>::Ptr, cv::Mat, int)),
              this,
              SLOT(extractionResultSlot(pcl::PointCloud<pcl::PointXYZI>::Ptr,
                                        cv::Mat, int)));

      calibvelodynesnormals[i] = loadSavedMat(
          fs, QString("%1_%2").arg(VELODYNENORMALS).arg(i).toStdString());
      ...
      calibvelodynenormalstab->addTab(tmpvelodynenormaltable,
                                      QString("Normal_%1").arg(i));
      setResultShow(calibvelodynesnormals[i], tmpvelodynenormaltable);
    }
    fs[QString("%1_%2").arg(CALIBRATIONERROR).arg("Rotation").toStdString()] >>
        calibrationrotationalerror;
    fs[QString("%1_%2")
           .arg(CALIBRATIONERROR)
           .arg("Translation")
           .toStdString()] >>
        calibrationtranslationalerror;
    calibrationerrorshow->setText(QString("%1, %2")
                                      .arg(calibrationrotationalerror)
                                      .arg(calibrationtranslationalerror));
  }
...
bool CalibrateCameraVelodyneChessboardBase::saveCalibResult(
    cv::FileStorage &fs) {
  CalibrateCameraChessboardBase::saveCalibResult(fs);

  QMessageBox::StandardButton button = QMessageBox::question(
      this, "Save Option", "Save Velodyne Calibration Data?");
  if (button == QMessageBox::Yes) {
    int i, n = calibvelodynespoints.size();
    printf("calibvelodynespoints.size() %d\n", calibvelodynespoints.size());
    for (i = 0; i < n; i++) {
      int j, m;
      cv::Mat tmpmat(m, 4, CV_32F);
      if (calibvelodynespoints[i] != NULL) {
        j, m = calibvelodynespoints[i]->points.size();
        for (j = 0; j < m; j++) {
      ...
      j, m = planeextractorspoints[i]->points.size();
      printf("i:%d planeextractorspoints[i]->points.size() %d\n", i,
             planeextractorspoints[i]->points.size());
      cv::Mat tmpmat1(m, 4, CV_32F);
      for (j = 0; j < m; j++) {
        tmpmat1.at<float>(j, 0) = planeextractorspoints[i]->points[j].x;
        tmpmat1.at<float>(j, 1) = planeextractorspoints[i]->points[j].y;
        tmpmat1.at<float>(j, 2) = planeextractorspoints[i]->points[j].z;
        tmpmat1.at<float>(j, 3) = planeextractorspoints[i]->points[j].intensity;
      }

      if (calibvelodynespoints[i] != NULL) {
        printf(
            "save: %s\n",
            QString("%1_%2").arg(VELODYNEPOINTS).arg(i).toStdString().c_str());
        fs << QString("%1_%2").arg(VELODYNEPOINTS).arg(i).toStdString()
           << tmpmat;
      }

      printf("save: %s\n", QString("%1_%2")
                               .arg(PLANEEXTRACTORSPOINTS)
                               .arg(i)
                               .toStdString()
                               .c_str());
      fs << QString("%1_%2").arg(PLANEEXTRACTORSPOINTS).arg(i).toStdString()
      ...
      printf(
          "save: %s\n",
          QString("%1_%2").arg(VELODYNENORMALS).arg(i).toStdString().c_str());
      fs << QString("%1_%2").arg(VELODYNENORMALS).arg(i).toStdString()
         << calibvelodynesnormals[i];
      PlaneExtractor *tmpviewer =
          (PlaneExtractor *)(calibvelodynesshow->widget(i));
      Eigen::Matrix4d viewerpose = tmpviewer->getCameraPose();
      cv::Mat tmpviewerpose = cv::Mat::eye(4, 4, CV_64F);
      tmpviewerpose.at<double>(0, 0) = viewerpose(0, 0);
      tmpviewerpose.at<double>(0, 1) = viewerpose(0, 1);
      tmpviewerpose.at<double>(0, 2) = viewerpose(0, 2);
      tmpviewerpose.at<double>(0, 3) = viewerpose(0, 3);
      tmpviewerpose.at<double>(1, 0) = viewerpose(1, 0);
      tmpviewerpose.at<double>(1, 1) = viewerpose(1, 1);
      tmpviewerpose.at<double>(1, 2) = viewerpose(1, 2);
      tmpviewerpose.at<double>(1, 3) = viewerpose(1, 3);
      tmpviewerpose.at<double>(2, 0) = viewerpose(2, 0);
      tmpviewerpose.at<double>(2, 1) = viewerpose(2, 1);
      tmpviewerpose.at<double>(2, 2) = viewerpose(2, 2);
      tmpviewerpose.at<double>(2, 3) = viewerpose(2, 3);
      printf("save: %s\n", QString("%1_%2")
                               .arg(VELODYNEVIEWERPOSE)
                               .arg(i)
                               .toStdString()
                               .c_str());
      fs << QString("%1_%2").arg(VELODYNEVIEWERPOSE).arg(i).toStdString()
...
CalibrateCameraVelodyneChessboardROS::CalibrateCameraVelodyneChessboardROS(
    ...
    float maxRange, cv::Size2f patternSize, cv::Size2i patternNum,
    QWidget *parent)
    : CalibrateCameraVelodyneChessboardBase(maxRange, patternSize, patternNum,
                                            parent) {
  camerasub = new ROSSub<sensor_msgs::ImageConstPtr>(
      cameraTopic, cameraQueueSize, cameraInterval);
  connect(camerasub, SIGNAL(receiveMessageSignal()), this,
          SLOT(refreshImageSlot()));
  velodynesub = new ROSSub<sensor_msgs::PointCloud2ConstPtr>(
      velodyneTopic, velodyneQueueSize, velodyneInterval);
  connect(velodynesub, SIGNAL(receiveMessageSignal()), this,
          SLOT(refreshVelodyneSlot()));
  camerasub->startReceiveSlot();
...
bool CalibrateCameraVelodyneChessboardROS::refreshVelodyne() {
  ...
  velodynetimestamp = QTime::fromMSecsSinceStartOfDay(
      (msg->header.stamp.sec % (24 * 60 * 60)) * 1000 +
      msg->header.stamp.nsec / 1000000);
  calibvelodyne = msg;
...
bool CalibrateCameraVelodyneChessboardROS::grabCalibData() {
  ...
  if (!found) {
    camerasub->startReceiveSlot();
    velodynesub->startReceiveSlot();
    printf("Can't find Chessboard Corners!\n");
    return 0;
  }
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr tmpptr1(
      new pcl::PointCloud<pcl::PointXYZI>);
  int i, n = calibvelodyne->height * calibvelodyne->width;
  tmpptr1->width = n;
  tmpptr1->height = 1;
  tmpptr1->is_dense = false;
  tmpptr1->points.resize(n);
  char *data = (char *)(calibvelodyne->data.data());
  for (i = 0; i < n; i++) {
    float *base = (float *)(data + i * calibvelodyne->point_step);
    tmpptr1->points[i].x = base[0];
    tmpptr1->points[i].y = base[1];
    tmpptr1->points[i].z = base[2];
    tmpptr1->points[i].intensity = base[4];
  }
  planeextractorspoints.push_back(tmpptr1);

  QTableWidget *tmpvelodynetable = new QTableWidget;
  calibvelodynepointstab->addTab(
      tmpvelodynetable,
      QString("Velodyne_%1").arg(calibvelodynespoints.size() - 1));

  calibvelodynesnormals.push_back(cv::Mat(1, 3, CV_64F));
  QTableWidget *tmpvelodynenormaltable = new QTableWidget;
  calibvelodynenormalstab->addTab(
      tmpvelodynenormaltable,
      QString("Normal_%1").arg(calibvelodynespoints.size() - 1));

  double xradius = patternsize.height * patternnum.height / 2;
  double yradius = patternsize.width * patternnum.width / 2;
  double radius = xradius < yradius ? xradius : yradius;
  radius = PLANEEXTRACTORRADIUS;
  PlaneExtractor *planeextraction = new PlaneExtractor(
      calibvelodyne, calibvelodynespoints.size() - 1, radius);
  calibvelodynesshow->addTab(
      planeextraction,
      QString("Velodyne_%1").arg(calibvelodynespoints.size() - 1));
  ...
  planeextraction->update();
  connect(planeextraction,
          SIGNAL(extractionResultSignal(pcl::PointCloud<pcl::PointXYZI>::Ptr,
                                        cv::Mat, int)),
          this,
          SLOT(extractionResultSlot(pcl::PointCloud<pcl::PointXYZI>::Ptr,
                                    cv::Mat, int)));
...
CalibrateCameraLidarChessboardBase::CalibrateCameraLidarChessboardBase(
    float maxRange, cv::Size2f patternSize, cv::Size2i patternNum,
    QWidget *parent)
    : CalibrateCameraChessboardBase(patternSize, patternNum, parent) {
  maxrange = maxRange;
...
void CalibrateCameraLidarChessboardBase::extractionResultSlot(
    QVector<QPointF> extraction, int id) {
  ...
  if (extraction.size() > 0) {
    ...
    for (i = 0; i < n; i++) {
      pointstable->setItem(
          i, 0, new QTableWidgetItem(QString("%1").arg(extraction[i].x())));
      pointstable->setItem(
          i, 1, new QTableWidgetItem(QString("%1").arg(extraction[i].y())));
    }
  }
...
bool CalibrateCameraLidarChessboardBase::removeCalibData() {
  ...
  QTableWidget *tmplidartable =
      (QTableWidget *)(caliblidarpointstab->widget(id));
  caliblidarpointstab->removeTab(id);
  delete tmplidartable;

  PointsExtractor *pointsextraction =
      (PointsExtractor *)(caliblidarsshow->widget(id));
...
double calibrationCameraLidarChessboardObjectiveFunc(
    const std::vector<double> &x, std::vector<double> &grad, void *data) {
  CalibrateCameraLidarChessboardBase::CameraLidarCalibrationData *calibdata =
      reinterpret_cast<
          CalibrateCameraLidarChessboardBase::CameraLidarCalibrationData *>(
          data);
  ...
  for (i = 0; i < n; i++) {
    ...
    cv::Mat duppoints = cv::Mat::ones(pointsnum, 1, CV_64F) * points.row(i);
    cv::Mat dists =
        (calibdata->lidarpoints[i] - duppoints) * (normals.row(i).t());
    cv::Mat tmperror = (dists.t()) * dists;
    double delta = tmperror.at<double>(0);
    error += delta;
  }
...
bool CalibrateCameraLidarChessboardBase::calibrateSensor() {
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < m; j++) {
      calibrationdata.lidarpoints[i].at<double>(j, 0) =
          double(caliblidarspoints[i][j].x());
      calibrationdata.lidarpoints[i].at<double>(j, 1) =
          double(caliblidarspoints[i][j].y());
...

  {
    ...
    opt.set_min_objective(calibrationCameraLidarChessboardObjectiveFunc,
                          (void *)&calibrationdata);
  ...
  cameraextrinsicmat.at<double>(0, 3) = x[3];
  cameraextrinsicmat.at<double>(1, 3) = x[4];
  cameraextrinsicmat.at<double>(2, 3) = x[5];
...
bool CalibrateCameraLidarChessboardBase::loadCalibResult(cv::FileStorage &fs) {
  ...
  QMessageBox::StandardButton button = QMessageBox::question(
      this, "Load Option", "Load LIDAR Calibration Data?");
  if (button == QMessageBox::Yes) {
    int i, n = (int)fs[CHESSBOARDVIEWNUM];
    ...
    for (i = 0; i < n; i++) {
      ...
      for (j = 0; j < m; j++) {
        ...
        tmplidartable->setItem(j, 0,
                               new QTableWidgetItem(QString("%1").arg(
                                   caliblidarspoints[i][j].x())));
        tmplidartable->setItem(j, 1,
                               new QTableWidgetItem(QString("%1").arg(
                                   caliblidarspoints[i][j].y())));
      }

      PointsExtractor *pointsextraction =
          new PointsExtractor(caliblidarspoints[i], i, 400, 10, 2);
      caliblidarsshow->addTab(pointsextraction, QString("LIDAR_%1").arg(i));
      connect(pointsextraction,
              SIGNAL(extractionResultSignal(QVector<QPointF>, int)), this,
              SLOT(extractionResultSlot(QVector<QPointF>, int)));
    }
...
bool CalibrateCameraLidarChessboardBase::saveCalibResult(cv::FileStorage &fs) {
  ...
  QMessageBox::StandardButton button = QMessageBox::question(
      this, "Save Option", "Save LIDAR Calibration Data?");
  if (button == QMessageBox::Yes) {
    int i, n = caliblidarspoints.size();
    for (i = 0; i < n; i++) {
      int j, m = caliblidarspoints[i].size();
...
CalibrateCameraLidarChessboardROS::CalibrateCameraLidarChessboardROS(
    QString cameraTopic, u_int32_t cameraQueueSize, int cameraInterval,
    QString lidarTopic, u_int32_t lidarQueueSize, int lidarInterval,
    float maxRange, cv::Size2f patternSize, cv::Size2i patternNum,
    QWidget *parent)
    : CalibrateCameraLidarChessboardBase(maxRange, patternSize, patternNum,
                                         parent) {
  camerasub = new ROSSub<sensor_msgs::ImageConstPtr>(
      cameraTopic, cameraQueueSize, cameraInterval);
  connect(camerasub, SIGNAL(receiveMessageSignal()), this,
          SLOT(refreshImageSlot()));
  lidarsub = new ROSSub<sensor_msgs::LaserScanConstPtr>(
      lidarTopic, lidarQueueSize, lidarInterval);
  connect(lidarsub, SIGNAL(receiveMessageSignal()), this,
          SLOT(refreshLidarSlot()));
  camerasub->startReceiveSlot();
...
bool CalibrateCameraLidarChessboardROS::refreshLidar() {
  ...
  lidartimestamp = QTime::fromMSecsSinceStartOfDay(
      (msg->header.stamp.sec % (24 * 60 * 60)) * 1000 +
      msg->header.stamp.nsec / 1000000);
  caliblidar = msg;
...
bool CalibrateCameraLidarChessboardROS::grabCalibData() {
  ...
  QTableWidget *tmplidartable = new QTableWidget;
  caliblidarpointstab->addTab(
      tmplidartable, QString("LIDAR_%1").arg(caliblidarspoints.size() - 1));

  PointsExtractor *pointsextraction = new PointsExtractor(
      caliblidar, caliblidarspoints.size() - 1, caliblidarviewer->imagesize,
      caliblidarviewer->maxrange, caliblidarviewer->gridsize);
  QScrollArea *scroll = new QScrollArea;
  scroll->setWidget(pointsextraction);
  caliblidarsshow->addTab(
      scroll, QString("LIDAR_%1").arg(caliblidarspoints.size() - 1));
