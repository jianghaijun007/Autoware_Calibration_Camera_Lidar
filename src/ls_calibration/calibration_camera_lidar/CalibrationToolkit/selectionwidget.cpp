...
PlaneExtractor::PlaneExtractor(sensor_msgs::PointCloud2ConstPtr velodynePoints,
                               int id, double neighborRadius,
                               double distanceThreshold, QWidget *parent)
    : GLViewer(parent) {
  pointsid = id;
  ...
  connect(this, SIGNAL(mousePositionSignal(QMouseEvent *, CAMERAPARAMETERS *)),
          this, SLOT(mousePositionSlot(QMouseEvent *, CAMERAPARAMETERS *)));
}

PlaneExtractor::PlaneExtractor(
    pcl::PointCloud<pcl::PointXYZI>::Ptr velodynePoints, int id,
    double neighborRadius, double distanceThreshold, QWidget *parent)
    : GLViewer(parent) {
  pointsid = id;
  neighborradius = neighborRadius;
  distance = distanceThreshold;
...
void PlaneExtractor::mousePositionSlot(QMouseEvent *event,
  ...
  gluUnProject(tmpx, iViewPort[3] - tmpy - 1, 1, dModelMatrix, dProjMatrix,
               iViewPort, &(tmpd[0]), &(tmpd[1]), &(tmpd[2]));
  dir(0) = tmpd[0];
  dir(1) = tmpd[1];
  dir(2) = tmpd[2];
  dir = dir - eye;
  ...
  do {
    ...
    for (i = 0; i < n; i++) {
      ...
      covariance(0, 0) = covariance(0, 0) + points[i](0) * points[i](0);
      covariance(0, 1) = covariance(0, 1) + points[i](0) * points[i](1);
      covariance(0, 2) = covariance(0, 2) + points[i](0) * points[i](2);
      covariance(1, 0) = covariance(1, 0) + points[i](1) * points[i](0);
      covariance(1, 1) = covariance(1, 1) + points[i](1) * points[i](1);
      covariance(1, 2) = covariance(1, 2) + points[i](1) * points[i](2);
      covariance(2, 0) = covariance(2, 0) + points[i](2) * points[i](0);
      covariance(2, 1) = covariance(2, 1) + points[i](2) * points[i](1);
      covariance(2, 2) = covariance(2, 2) + points[i](2) * points[i](2);
    }
  ...
  // printf("dis:%0.2f tmpmaxdis:%0.2f N:%d extracted:%d\n", dis, tmpmaxdis, N,
  // extracted);
  if (dis <= tmpmaxdis) {
    ...
    // printf("button:%d\n nearestpoint:[%0.2f %0.2f %0.2f]\n", event->button(),
    // cloud[k_indices[0]].x, cloud[k_indices[0]].y, cloud[k_indices[0]].z);
    switch (event->button()) {
      case Qt::LeftButton:
        if (!extracted) {
      ...
      case Qt::RightButton: {
        ...
        emit extractionResultSignal(extraction, cv::Mat(1, 3, CV_64F),
                                    pointsid);
        this->makeCurrent();
      ...
      case Qt::NoButton:
        drawMouse(point, eigenvectors.col(0), eigenvectors.col(2),
                  nearestpoint);
        // printf("drawMouse\n");
        break;
      default:
...
void PlaneExtractor::drawMouse(Eigen::Vector3d point, Eigen::Vector3d norm,
...
void PlaneExtractor::extractPlane(Eigen::Vector3d seed,
                                  Eigen::Matrix3d eigenvectors) {
  std::vector<int> k_indices;
  std::vector<float> k_sqr_distances;
  pcl::PointXYZI tmpseed;
  tmpseed.x = float(seed(0));
  tmpseed.y = float(seed(1));
  tmpseed.z = float(seed(2));
  std::vector<int> tmpresult;
  int n =
      kdtree.radiusSearchT(tmpseed, neighborradius, k_indices, k_sqr_distances);
  int i;
  for (i = 0; i < n; i++) {
    int index = k_indices[i];
    Eigen::Vector3d temppoint(double(cloud[index].x), double(cloud[index].y),
                              double(cloud[index].z));
  ...
  if (tmpresult.size() > 0) {
    ...
    for (i = 0; i < n; i++) {
      glVertex3f(extraction->points[i].x, extraction->points[i].y,
                 extraction->points[i].z);
    }
    glEnd();
    glEndList();
    this->update();
...
PointsExtractor::PointsExtractor(int imageSize, double maxRange,
                                 double gridSize) {
  pointsid = -1;
  imagesize = imageSize;
  maxrange = maxRange;
...
PointsExtractor::PointsExtractor(sensor_msgs::LaserScanConstPtr lidarPoints,
                                 int id, int imageSize, double maxRange,
                                 double gridSize) {
...
PointsExtractor::PointsExtractor(QVector<QPointF> lidarPoints, int id,
                                 int imageSize, double maxRange,
                                 double gridSize) {
  points = lidarPoints;
  drawPoints();
  pointsid = id;
...
void PointsExtractor::updateLaserScan(
    sensor_msgs::LaserScanConstPtr lidarPoints) {
  pointsptr = lidarPoints;
  int i, pointsnum = (pointsptr->angle_max - pointsptr->angle_min) /
                         pointsptr->angle_increment +
                     1;
  points.resize(pointsnum);
  for (i = 0; i < pointsnum; i++) {
    float angle = pointsptr->angle_min + i * pointsptr->angle_increment;
    points[i] = QPointF(pointsptr->ranges[i] * cos(angle),
                        pointsptr->ranges[i] * sin(angle));
  }
  drawPoints();
}
...
void PointsExtractor::drawPoints() {
  ...
  for (i = 1; i <= n; i++) {
    painter.drawEllipse(center, int(gridsize * i * ratio),
                        int(gridsize * i * ratio));
  }
...
void PointsExtractor::drawRectangle() {
  ...
  int startx =
      startcorner.x() < endcorner.x() ? startcorner.x() : endcorner.x();
  int starty =
      startcorner.y() < endcorner.y() ? startcorner.y() : endcorner.y();
