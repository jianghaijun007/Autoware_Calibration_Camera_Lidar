...
#define PLANEEXTRACTORSPOINTS "PlaneExtractorsPoints"
...
#define PLANEEXTRACTORRADIUS 0.6  // 0.2 // 0.6
#define USINGFIXEDINTRINSTICMATRIX 0
#define PATTERNSIZE 0.2     // 0.045 // 0.024; //0.107 //0.2
#define PATTERNNUMWIDTH 4   // 4 // 8
#define PATTERNNUMHEIGHT 3  // 3 // 6
...
class CalibrateCameraBase : public CalibrationToolkitBase {
 ...
 public:
  ...
  cv::Mat loadSavedMat(cv::FileStorage &fs, std::string matName);
};

class CalibrateCameraChessboardBase : public CalibrateCameraBase {
  Q_OBJECT
 public:
  CalibrateCameraChessboardBase(cv::Size2f patternSize, cv::Size2i patternNum,
                                QWidget *parent = 0);
 ...
 protected:
  ...
  enum CHESSBOARDTYPE { BoxGrid, CircleGrid };
 ...
 public:
  ...
  cv::Mat loadSavedMat(cv::FileStorage &fs, std::string matName);
  cv::Mat loadSavedImg(cv::FileStorage &fs, std::string matName);
};

class CalibrateCameraChessboardROS : public CalibrateCameraChessboardBase {
  Q_OBJECT
 public:
  CalibrateCameraChessboardROS(QString topic, u_int32_t queueSize, int interval,
...
class CalibrateCameraVelodyneChessboardBase
    : public CalibrateCameraChessboardBase {
  Q_OBJECT
 public:
  CalibrateCameraVelodyneChessboardBase(float maxRange, cv::Size2f patternSize,
                                        cv::Size2i patternNum,
                                        QWidget *parent = 0);
 ...
 protected:
  ...
  QVector<pcl::PointCloud<pcl::PointXYZI>::Ptr> planeextractorspoints;
 ...
 public slots:
  void extractionResultSlot(pcl::PointCloud<pcl::PointXYZI>::Ptr extraction,
                            cv::Mat normal, int id);
...
class CalibrateCameraVelodyneChessboardROS
    : public CalibrateCameraVelodyneChessboardBase {
  Q_OBJECT
 public:
...
class CalibrateCameraLidarChessboardBase
    : public CalibrateCameraChessboardBase {
  Q_OBJECT
 public:
  CalibrateCameraLidarChessboardBase(float maxRange, cv::Size2f patternSize,
                                     cv::Size2i patternNum,
                                     QWidget *parent = 0);
...
class CalibrateCameraLidarChessboardROS
    : public CalibrateCameraLidarChessboardBase {
  Q_OBJECT
 public:
  CalibrateCameraLidarChessboardROS(QString cameraTopic,
                                    u_int32_t cameraQueueSize,
                                    int cameraInterval, QString lidarTopic,
                                    u_int32_t lidarQueueSize, int lidarInterval,
                                    float maxRange, cv::Size2f patternSize,
                                    cv::Size2i patternNum, QWidget *parent = 0);
