...
class PlaneExtractor : public GLViewer {
  Q_OBJECT
 public:
  PlaneExtractor(sensor_msgs::PointCloud2ConstPtr velodynePoints, int id,
                 double neighborRadius = 0.2, double distanceThreshold = 0.05,
                 QWidget *parent = 0);
  PlaneExtractor(pcl::PointCloud<pcl::PointXYZI>::Ptr velodynePoints, int id,
                 double neighborRadius = 0.2, double distanceThreshold = 0.05,
                 QWidget *parent = 0);
 ...
 signals:
  void extractionResultSignal(pcl::PointCloud<pcl::PointXYZI>::Ptr extraction,
                              cv::Mat normal, int id);

 protected:
  void drawMouse(Eigen::Vector3d point, Eigen::Vector3d norm,
                 Eigen::Vector3d direction, Eigen::Vector3d nearestpoint);
...
class PointsExtractor : public QLabel {
 ...
 public:
  ...
  PointsExtractor(sensor_msgs::LaserScanConstPtr lidarPoints, int id,
                  int imageSize, double maxRange, double gridSize);
  PointsExtractor(QVector<QPointF> lidarPoints, int id, int imageSize,
                  double maxRange, double gridSize);
